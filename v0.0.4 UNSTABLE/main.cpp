...
#include <cstdlib>
#include <string>
#include <termios.h>//ta biblioteka oraz ponizsza pozwalaja na stworzenie funkcji mygetch()
#include <unistd.h>
...
int mygetch() //WAZNE!!! funkcja pozwala na uzywanie funckji getch; przykladowe wywolanie: mygetch();
{
        struct termios oldt,
        newt;
        int ch;
...
int wspolrzedne_gracza_po_wcisnieciu_klawisza(/*char kierunek_poruszania  */int aktualna_wysokosc, int aktualna_szerokosc, bool ktore_dane_zwracac) // 0x0 to jest gorny lewy rog terminala! dlatego jest -- oraz ++
{
    ...
    char kierunek_poruszania;
    kierunek_poruszania = mygetch();

    switch(kierunek_poruszania) //dowaliles z tym sterowaniem xD
    {
        case 'f': // forward // 0x66
        obliczone_wspolrzedne_gracza[0] = aktualna_wysokosc++;
        break;

        case 'b': // backward // 0x62
        obliczone_wspolrzedne_gracza[0] = aktualna_wysokosc--;
        break;
